...
int main(void)
{
	...
	rprintf("Initializing Ethernet Interface\r\n");
	nicInit();
	// initialize ARP
	rprintf("Initializing ARP cache\r\n");
	arpInit();
	// initialize IP addressing
	rprintf("Initializing IP Addressing\r\n");
	ipSetAddress(IPADDRESS, NETMASK, GATEWAY);
	nicGetMacAddress(&myEthAddress.addr[0]);
	arpSetAddress(&myEthAddress, ipGetMyAddress());
	
	rprintfProgStrM("Eth Addr is: "); netPrintEthAddr(&myEthAddress);	rprintfCRLF();
	rprintfProgStrM("IP  Addr is: "); netPrintIPAddr(ipGetMyAddress());	rprintfCRLF();
...
void serviceLocal(void)
{
	...
	if( (c = uartGetByte()) != -1)
	{
		...
		case 'u':
			...
			udpSend((IPADDRESS|0x000000FF), CONTROL_PORT, 6, &buffer[ETH_HEADER_LEN+IP_HEADER_LEN+UDP_HEADER_LEN]);
			break;
		case '?':
			rprintfProgStrM("\r\nCommands:\r\n");
