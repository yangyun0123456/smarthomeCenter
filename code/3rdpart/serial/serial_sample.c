...
static int handle_command(char* cmd, int len)
{
    ...
    unsigned long id = Read32(cmd[2], cmd[3], cmd[4], cmd[5]);
    unsigned long passwordH = Read32(cmd[6], cmd[7], cmd[8], cmd[9]);
    unsigned long passwordL = Read32(cmd[10], cmd[11], cmd[12], cmd[13]);
...
int main(int argc, char **argv)
{
    ...
    pthread_create(&threadId, NULL, readWriteThread, NULL);
