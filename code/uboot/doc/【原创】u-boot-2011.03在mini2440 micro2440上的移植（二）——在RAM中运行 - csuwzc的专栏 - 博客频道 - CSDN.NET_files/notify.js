(function(window, undefined) {
  ...
  var defaults = {
    realtime: 'http://s{0}.im.notify.csdn.net'
    , api: 'http://svc.notify.csdn.net'
    , app: 'http://notify.csdn.net'
    , space: 'http://my.csdn.net/'
    , count: 5
    , subCount: 5
    , staticUrl: 'http://csdnimg.cn/rabbit/notev2/'
  ...
  csdn.note.prototype = {
    ...
    staticUrl: function (url) {
      ...
      return this.conf.staticUrl + url + '?8b6dcf4';
    },
    ...
    changeToNickname: function (ctx) {
      ...
      if(users[0]) {
        $.getJSON('http://api.csdn.net/service/open/nick?callback=?', {
    ...
    addEvent: function() {
      ...
      $('.notice_list_con .notice_content', self.Dom.wrap).delegate('.list', 'click', function () {
        var item = $(this)
          , index = item.index()
...
<div class="error"></div>');
    ...
    fillList: function(insert, data) {
      ...
      $.each(data, function (i, v) {
        list[i] = '\
<dl class="list rev_type' + v.type + (/*parseInt(v.isReaded, 10)*/ insert === 'append' ? '' : ' unread') + '" style="display: none">\
  <dt>\
    <i></i>\
...
<div style="display: none">\
  <dl class="rev_type' + v.type + '" data-ids="' + v.containIds.join() + '">\
    <dt>\
      <i></i>\
  ...
  <a class="notifications_more" target="_blank">查看其它 0 条</a>\
    ...
    checkLogin: function(callback) {
      $.getScript("http://ptcms.csdn.net/comment/js/login.js", function(data) {
    ...
    */
    feTemplate : function(tempStr){
      // TODO 未处理模板字段重复出现的问题
      //      未处理返回的模板对象复用的问题
      // 测试用例 http://jsfiddle.net/SdzfU/2/
      return {
          render:function(conf){
            for(var name in conf){
                if(conf.hasOwnProperty(name)){
                    tempStr=tempStr.replace("{%"+name+"%}",conf[name]);
                }
            }
            return tempStr;
          }
